...
& (Join-Path $root 'build-exe-346.ps1')
...
& $csc /codepage:65001 /target:winexe /platform:anycpu /win32icon:"$root\app-icon.ico" /out:"$payload\Uninstall Jangmao70.exe" /reference:System.Windows.Forms.dll /reference:Microsoft.CSharp.dll "$root\Document346Uninstaller.cs"
if ($LASTEXITCODE -ne 0) { throw "Uninstaller compile failed with exit code $LASTEXITCODE" }
& $csc /codepage:65001 /target:winexe /platform:anycpu /win32icon:"$root\app-icon.ico" /out:"$release\Jangmao70-Setup.exe" /reference:System.Windows.Forms.dll /reference:Microsoft.CSharp.dll "$root\Document346Installer.cs"
if ($LASTEXITCODE -ne 0) { throw "Installer compile failed with exit code $LASTEXITCODE" }

Copy-Item -LiteralPath (Join-Path $root 'USER_README_346_TH.txt') -Destination (Join-Path $release 'README-TH.txt') -Force
