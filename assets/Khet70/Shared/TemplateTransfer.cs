...
namespace Khet70
{
    ...
    internal static class TemplateTransferService
    {
        ...
        {
            "school.district", "school.districtShort",
            ...
            "school.financeDirector.prefix", "school.financeDirector.given", "school.financeDirector.surname",
            "school.deputyDistrict.prefix", "school.deputyDistrict.given", "school.deputyDistrict.surname",
            "employee.prefix", "employee.given", "employee.surname", "employee.position", "employee.salary", "employee.nationalId",
        ...
        private static readonly Dictionary<string, string> ProcurementPositionToPayroll = LoadProcurementPositionToPayroll();

        private static readonly Dictionary<string, string> PayrollPositionToProcurement = ProcurementPositionToPayroll.ToDictionary(x => x.Value, x => x.Key, StringComparer.Ordinal);

        private static readonly Dictionary<string, string> PayrollPositionAliasesToProcurement = LoadProcurementPositionAliases();

        private static Dictionary<string, string> LoadProcurementPositionToPayroll()
        {
            ...
            try
            {
                ...
                foreach (var position in catalog.positions ?? new ProcurementPositionConfig[0])
                {
                    ...
                    result[position.id] = string.IsNullOrWhiteSpace(position.dropdownLabel) ? position.label : position.dropdownLabel;
                }
            }
            catch { }
            return result;
        ...
        private static Dictionary<string, string> LoadProcurementPositionAliases()
        {
            var result = new Dictionary<string, string>(StringComparer.Ordinal);
            try
            {
                ...
                {
                    if (position == null || string.IsNullOrWhiteSpace(position.id)) continue;
                    if (!string.IsNullOrWhiteSpace(position.label)) result[position.label] = position.id;
                }
            }
            catch { }
            return result;
        ...
        public static List<TemplateTransferItem> LoadPayrollTemplates()
        {
            ...
            var paths = new[]
            {
                Khet70Paths.SavedTemplatesPathFor(DocumentModule.Payroll),
        ...
        public static List<TemplateTransferItem> LoadProcurementTemplates()
        {
            try
            {
                var data = new Khet70Store(DocumentModule.Procurement).Load();
        ...
        private static void AddPayrollCommon(TemplateTransferItem item, Dictionary<string, string> data)
        {
            ...
            AddPayroll(item, "school.districtShort", data, "{ชื่อเขตย่อ}");
            ...
            AddPerson(item, "school.financeDirector", data, "ผอกง");
            AddPerson(item, "school.deputyDistrict", data, "รองเขต");
            ...
            AddPayroll(item, "employee.salary", data, "{เงินเดือน}");
        ...
        private static void AddProcurementCommon(Dictionary<string, string> values, WorkingRecord record)
        {
            ...
            Add(values, "school.districtShort", record.School == null ? "" : record.School.DistrictShort);
            ...
            AddPerson(values, "school.financeDirector", record.School == null ? null : record.School.FinanceDirector);
            AddPerson(values, "school.deputyDistrict", record.School == null ? null : record.School.DeputyDistrictDirector);
            ...
            Add(values, "employee.salary", record.Procurement == null ? "" : record.Procurement.Salary);
        ...
        private static string FriendlyLabel(string key)
        {
            ...
            {
                { "school.name", "ข้อมูลหน่วยงานเดิม (ไม่ใช้)" },
                { "school.district", "เขตพื้นที่การศึกษา" },
                { "school.districtShort", "ชื่อเขตย่อ" },
                { "school.director.prefix", "คำนำหน้าผู้อำนวยการสำนักงานเขตพื้นที่" },
                { "school.director.given", "ชื่อผู้อำนวยการสำนักงานเขตพื้นที่" },
                { "school.director.surname", "นามสกุลผู้อำนวยการสำนักงานเขตพื้นที่" },
                ...
                { "school.financeDirector.prefix", "คำนำหน้าผู้อำนวยการกลุ่มการเงินและสินทรัพย์" },
                { "school.financeDirector.given", "ชื่อผู้อำนวยการกลุ่มการเงินและสินทรัพย์" },
                { "school.financeDirector.surname", "นามสกุลผู้อำนวยการกลุ่มการเงินและสินทรัพย์" },
                { "school.deputyDistrict.prefix", "คำนำหน้ารองผู้อำนวยการเขตพื้นที่" },
                { "school.deputyDistrict.given", "ชื่อรองผู้อำนวยการเขตพื้นที่" },
                { "school.deputyDistrict.surname", "นามสกุลรองผู้อำนวยการเขตพื้นที่" },
                { "employee.position", "ตำแหน่ง" },
                { "employee.salary", "เงินเดือน" },
